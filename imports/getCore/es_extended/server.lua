...
return function(resource)
	...
	function CPlayer:hasGroup(filter)
		...
		if type == 'string' then
			...
			elseif self.faction.name == filter then
				return self.faction.name, self.faction.grade
			end
		else
			...
			if tabletype == 'hash' then
				...
				local grade2 = filter[self.faction.name]

				if grade and grade <= self.job.grade then
					return self.job.name, self.job.grade
				elseif grade2 and grade2 <= self.faction.grade then
					return self.faction.name, self.faction.grade
				end
			elseif tabletype == 'array' then
				for i = 1, #filter do
					if self.job.name == filter[i] then
						return self.job.name, self.job.grade
					elseif self.faction.name == filter[i] then
						return self.faction.name, self.faction.grade
					end
				end
			end
